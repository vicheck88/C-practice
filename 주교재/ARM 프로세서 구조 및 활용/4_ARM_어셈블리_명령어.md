...
- 비트 연산을 통해 제어하고자 하는 부분만 제어할 수 있음
...
- 어셈블러는 =상수를 ldr,mov,mvn 3가지 명령으로 자동 전환
...
- C: Carry 발생여부, V: Overflow 발생여부
    - Overflow와 Carry
        - Carry는 최상위 비트에서 값 넘김이 발생할 때
        - Overflow는 최상위 비트와 그 전 비트에서의 값 넘김 여부가 다를 경우
			- Carry가 발생했지만 그 전 비트에서 값 넘김이 없을 경우
			- Carry가 발생하지 않았지만 그 전 비트에서 값 넘김이 있을 경우
